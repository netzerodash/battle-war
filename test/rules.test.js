...
test('starting armies keep their intended size across the three-ring city', () => {
  ...
  // 3,764 หน่วย = 12.81 ms/เฟรม (เกมเดิม 3,373 หน่วย = 12.02) ยังพอมีงบให้เรนเดอร์ที่ 60fps
  // ถ้าจะดันเกินนี้ ต้องวัดใหม่ก่อน เพราะต้นทุนขึ้นกับ "ความหนาแน่น" มากกว่าจำนวนรวม:
  // กองสำรอง 600 นายในวงนอก = 15.00 ms · องครักษ์ที่อัดในวงชั้นใน (รัศมี 38/18) แพงที่สุด
  // — 3,924 หน่วยแบบองครักษ์หนา = 15.92 ms ทั้งที่จำนวนรวมพอกับ 3,934 หน่วยที่ 15.00 ms
  assert.ok(attackers + defenders <= 3800, `total units ${attackers + defenders}`);
});
