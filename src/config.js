...
export const CFG = {
  ...
  // สเตตัสรายนายคงค่าเดิมทั้งหมด — npm run sim พิสูจน์แล้วว่าการบัฟ hp/dmg ฝ่ายเรา
  // คือสิ่งที่ทำให้เกมพัง (บอทรุมมั่วพุ่งจาก 1/4 เป็น 4/4 ทหารรอด 16%→69% และ
  // การวางแผนหมดความหมายเพราะบอทวางแผนกับบอทรุมมั่วได้ผลเท่ากัน) ปัญหา "ฝ่าชั้นใน
  // ไม่ได้" มาจากบั๊กพลธนูอมตะล้วน ๆ พอแก้บั๊กแล้วสเตตัสเดิมก็เพียงพอ
  unit: {
  ...
  // ---------- ฝ่ายรับ ----------
  // ชดเชยทัพบุกที่ใหญ่ขึ้นด้วยกำลังบนกำแพงเป็นหลัก ไม่ใช่กองสำรอง — กองสำรองยืนกระจุก
  // เป็นวงจึงแพงในลูปปะทะมาก (600 นายดันต้นทุนจำลองจาก 11 เป็น 15 ms/เฟรม)
  // ส่วนทหารกำแพงกระจายยาวตาม 4 ด้าน เพิ่มได้ถูกกว่ามากที่ความยากเท่ากัน
  wallMelee: 132,
  wallArchers: 28,
  archerCd: 2.4,
  archerRange: 45,          // ไม่ถึงแนวตั้งทัพเริ่มต้น แต่ยิงโต้ธนูที่เข้าประจำตำแหน่งได้
  // hp 14 (เดิม 11) หักล้างทัพบุกที่ใหญ่ขึ้น ~25% โดยไม่กินงบจำนวนหน่วย:
  // หอกเราใช้เวลาฆ่านานขึ้น 27% (10.5→13.3 วิ) เวลายึดกำแพงจึงใกล้เคียงของเดิม
  // เลือกทางนี้เพราะการชดเชยด้วยจำนวนชนเพดานเฟรมเรตแล้ว (ต้องใช้ทหารเมือง ~1,785
  // นาย รวม 4,221 หน่วย ≈ 16-17 ms/เฟรม) และโจทย์กำหนดว่าห้ามแตะองครักษ์ชั้นใน
  defender: { hp: 14, dmg: 1, atkCd: 1.3, speed: 2.4 },
  ...
  // ลดกองสำรองมาจ่ายเป็นงบให้องครักษ์ชั้นใน (ดูคอมเมนต์ garrison ด้านล่าง) — ย้ายความแข็ง
  // ของเมืองจากวงนอกที่เป็นแค่การบดทัพทิ้ง ไปไว้ที่ชั้นในซึ่งเป็นไคลแมกซ์ของศึก
  reserveSquads: 60,
  squadSize: 4,
  ...
  garrison: {
    ...
    // จำนวนองครักษ์ +40% (สเตตัสรายนายคงเดิมทุกตัว) — ชดเชยทัพบุกที่ใหญ่ขึ้น 25%
    // ตรงจุดที่ควรเป็นไคลแมกซ์ของศึก และกินงบจำนวนหน่วยแค่ +113 นาย ถูกกว่าการเพิ่ม
    // ทหารกำแพงหรือกองสำรองมาก เพราะฐานเดิมของทั้งสามชั้นในรวมกันมีแค่ 282 นาย
    inner: { shields: 59, spears: 59, cav: 36, archersPerSide: 20 },
    palace: { shields: 39, spears: 48, cav: 25, archersPerSide: 13 },
